...
async function authorize(db, code, memberId, token) {
  ...
  return db.prepare("SELECT member_id, name, is_host FROM room_members WHERE room_code = ? AND member_id = ? AND token_hash = ?").bind(code, memberId, tokenHash).first();
}
...
async function drinkEventsForRoom(db, code) {
  const result = await db.prepare(
    "SELECT id, sender_id, target_id, rank, drinks, status, shown_card_json, created_at, resolved_at FROM room_drink_events WHERE room_code = ? ORDER BY created_at DESC LIMIT 40"
  ).bind(code).all();
...
async function roomPayload(db, room) {
  return {
    ...
    state: parseState(room.state_json),
    members: await membersForRoom(db, room.code),
    drinkEvents: await drinkEventsForRoom(db, room.code),
  };
}
...
async function createRoom(db, request) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    const existing = await getRoom(db, candidate);
    if (!existing) {
      code = candidate;
      break;
    }
  ...
  const payload = await roomPayload(db, room);
  return json({ ...payload, token, member: { id: memberId, name, host: true } }, 201);
}
...
async function joinRoom(db, request, code) {
  ...
  if (state.screen && state.screen !== "roomlobby") return fail("room_already_started", 409);
  const members = await membersForRoom(db, code);
  if (members.length >= MAX_ROOM_MEMBERS) return fail("room_full", 409);
  const now = Date.now();
  const memberId = randomToken().slice(0, 16);
  const token = randomToken();
  await db.prepare("INSERT INTO room_members (room_code, member_id, token_hash, name, is_host, joined_at) VALUES (?, ?, ?, ?, 0, ?)").bind(code, memberId, await hashToken(token), name, now).run();
  const payload = await roomPayload(db, room);
  return json({ ...payload, token, member: { id: memberId, name, host: false } }, 201);
}

async function readRoom(db, request, url, code) {
  const room = await getRoom(db, code);
  if (!room) return fail("room_not_found", 404);
  const member = await authorize(db, code, url.searchParams.get("member") || "", getToken(request));
...
async function updateRoomState(db, request, code) {
  ...
  if (!state || typeof state !== "object" || Array.isArray(state)) return fail("invalid_room_state");
  const serialized = JSON.stringify(state);
  if (serialized.length > MAX_STATE_BYTES) return fail("room_state_too_large", 413);
  const version = Number(body?.version);
  if (!Number.isInteger(version)) return fail("invalid_room_version");
  const result = await db.prepare("UPDATE rooms SET state_json = ?, version = version + 1 WHERE code = ? AND version = ?").bind(serialized, code, version).run();
  if (!result.meta?.changes) return fail("room_state_conflict", 409);
  const updated = await getRoom(db, code);
  return json({ code, version: Number(updated.version) });
}
...
async function createDrinkEvent(db, request, code) {
  ...
  if (state.screen !== "play" || state.phase === "flip" || !card?.rank || !Number.isFinite(Number(card.drinks))) {
    return fail("drink_event_not_available", 409);
  }

  const targetId = String(body?.target || "");
  const members = await membersForRoom(db, code);
  if (!targetId || targetId === member.member_id || !members.some((item) => item.id === targetId)) {
    return fail("invalid_drink_target");
  }
  ...
  const event = {
    ...
    targetId,
    rank: String(card.rank).slice(0, 8),
    drinks: Math.max(1, Math.floor(Number(card.drinks))),
    status: "pending",
    shownCard: null,
    createdAt: Date.now(),
    resolvedAt: null,
  ...
  await db.prepare(
    "INSERT INTO room_drink_events (id, room_code, sender_id, target_id, rank, drinks, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  ).bind(event.id, code, event.senderId, event.targetId, event.rank, event.drinks, event.status, event.createdAt).run();
...
async function respondToDrinkEvent(db, request, code, eventId) {
  ...
  const event = await db.prepare(
    "SELECT id, sender_id, target_id, rank, drinks, status FROM room_drink_events WHERE id = ? AND room_code = ?"
  ...
  if (decision === "challenge") {
    ...
    const hand = senderIndex >= 0 ? parseState(room.state_json).hands?.[senderIndex] : null;
    const cards = Array.isArray(hand) ? hand : [];
    shownCard = cards.find((card) => String(card?.rank || "") === String(event.rank)) || cards[0] || null;
    status = shownCard && String(shownCard.rank || "") === String(event.rank) ? "truth" : "lie";
  }

  const resolvedAt = Date.now();
  const result = await db.prepare(
    "UPDATE room_drink_events SET status = ?, shown_card_json = ?, resolved_at = ? WHERE id = ? AND room_code = ? AND status = 'pending'"
  ).bind(status, shownCard ? JSON.stringify(shownCard) : null, resolvedAt, eventId, code).run();
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" || url.pathname === "/privacy" || url.pathname === "/privacy-policy") return textResponse(html);
    if (!url.pathname.startsWith("/api/rooms")) return new Response("Not found", { status: 404, headers: { "content-type": "text/plain; charset=utf-8" } });
